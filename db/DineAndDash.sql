...
CREATE TABLE DineAndDash_DeliveryTracking (
    tracking_id INT PRIMARY KEY IDENTITY(1,1),
    order_id INT,
    rider_id INT NULL,
    status VARCHAR(20) CHECK (status IN ('Searching','Assigned', 'Picked Up', 'Out for Delivery', 'Delivered')) DEFAULT 'Searching',
    last_updated DATETIME DEFAULT GETDATE(),
    FOREIGN KEY (order_id) REFERENCES DineAndDash_Orders(order_id) ON DELETE CASCADE,
    FOREIGN KEY (rider_id) REFERENCES DineAndDash_Users(user_id) ON DELETE CASCADE
);
